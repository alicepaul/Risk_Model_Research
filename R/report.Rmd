...
In order to create a risk score model, we must have a binary outcome with integer (or near-integer) covariates. Here, we are interested in medication adherence (`PCTadherence`), which is a continuous variable that measures the percentage of doses taken on time. To make this outcome binary, we'll need to define a cutoff above which we'll consider the patient "adherent". Table 1 explores the class imbalance when using a cutoff of 90% (where a patient must have 90% adherence to be considered "adherent"). We observe class imbalance, with only 8.4% of participants classified as "non-adherent" (n = 21) and 91.6% of participants classified as "adherent" (n = 228). 


```{r, eval = FALSE}

data.frame(non_adherent = c(sum(raw_data$PCTadherence < 90, na.rm = TRUE),
                           sum(raw_data$PCTadherence_sensi < 90, na.rm  = TRUE)),
           adherent = c(sum(raw_data$PCTadherence > 90, na.rm = TRUE),
                           sum(raw_data$PCTadherence_sensi > 90, na.rm  = TRUE)),
  ...
  kable(format = "latex", caption = "Patient Count by Dichotomized Outcome", 
                  col.names = c("<90% Adherence", ">90% Adherence", "Missing"),
                  booktabs = TRUE) # %>% 
...
The raw data contained 266 observations for each regimen in the study. The study included 249 individual participants and some had multiple regimens. For this analysis we considered only the first regimen for each participant. After removing the subsequent regimens, the data contained one row for each participant (n = 249). 
...
Distributions of the continuous covariates are visualized in the appendix. The variables `self_eff`, `stig_tot`, and `phq9_tot` were dropped There were four continuous variables that were converted to categorical using the following cutoffs (selected based on class balance): 
...
The categorical variables `ram` and `regular_drug` were dropped because they each had only 5 patients responding with "yes". The variables `edu_level_mom` and `edu_level_dad` could potentially be combined, but they contained many "I don't know" responses so  were dropped. The `monitor1` variable was also dropped because it was not clear how to relevel it. 
...
cv_results <- cv_risk_mod(X, y, foldids = folds, a = -5, b = 5, seed = 1)
...
```{r, warning = FALSE}
lambda0 <- cv_results$results$lambda0[which(cv_results$results$nonzero == 4)]

mod <- risk_mod(X, y, lambda0 = lambda0, a = -5, b = 5)
...
data.frame(mod$model_card, row.names = c("Lives with Mom (0/1)", 
  ...
  kable(format = "latex", caption = "Score Card" , 
...
data.frame(Score = as.character(score_range), 
  ...
  kable(align = "c", format = "latex", caption = "Score-Risk Map" , 
                  booktabs = TRUE)#  %>% 
...
ggplot() + 
  ...
  geom_function(data = data.frame(x = seq(-5,15)), aes(x), 
