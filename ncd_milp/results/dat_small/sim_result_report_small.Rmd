...
milp_v2 <- read_csv('results_milp_v2_testing_2.24_t_limit.csv')

# parameter columns
milp_v2 <- milp_v2 %>% mutate(p1 = unlist(lapply(milp_v2$data, function(input_string) {
  split_string <- unlist(strsplit(input_string, "_"))
  return(split_string[3])
  })), 
                                   p2 = unlist(lapply(milp_v2$data, function(input_string) {
  split_string <- unlist(strsplit(input_string, "_"))
  return(split_string[4])
  })),
                                   eps = unlist(lapply(milp_v2$data, function(input_string) {
  split_string <- unlist(strsplit(input_string, "_"))
  return(split_string[5])
  })),
                                   link = unlist(lapply(milp_v2$data, function(input_string) {
  split_string <- unlist(strsplit(input_string, "_"))
  return(split_string[6])
  })))
...
```{r}
# acc comparison
acc_dat <- as.data.frame(cbind(new_alg$acc,ncd$acc,logis$acc,milp_v2$acc ,new_alg$link)) %>%
